// =========== Wirtgen Alert Center — Frontend Server ============
'use strict';
...
app.use('/api', (req, res) => {
  ...
  proxyReq.on('error', (err) => {
    console.error('[WAC-PROXY] Erro:', err.message);
    if (!res.headersSent) {
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(`[WAC-FRONTEND] Rodando na porta ${PORT}`);
  console.log(`[WAC-FRONTEND] Proxy /api → ${BACKEND_URL}`);
});
