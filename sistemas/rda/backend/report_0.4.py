...
# REPORT AUTOMATICO DE DESEMPENHO - RDA
# DESENVOLVIDO POR THIAGO BARROS - thiago.barros@venezanet.com - 2026.1
# VERSÃO ESTÁVEL - 0.4 - Data 16/06/2026 -
# -> Alterações: Correção da geração de páginas vazias nos relatórios de perfomance e utilização
# =========================================================================================
...
#Listas de ID Padrões para o fluxo de Execução
...
MENSAGEM_SEM_DADOS = " "
MENSAGEM_SEM_DADOS_TABELA = "Sem Dados"
...
CORES_ALERTA = {
    ...
    "Alertas de Atenção": AMARELO_ALERTA,
...
ARQUIVOS_SECOES_PDF = {
    ...
    "arquivo_oleo": "AMOSTRAS_DE_OLEO.pdf",
    "arquivo_teste": "A3_AMOSTRAS_DE_OLEO.pdf",
    "analise_garantia": "SEC_09_GARANTIA.pdf",
    "encerramento": "SEC_10_ENCERRAMENTO.pdf",

}

# === PDFs temporarios por seção do relatório por pin

ARQUIVOS_SECOES_PDF_BY_PIN = {

    "capa": "SEC_01_CAPA.pdf",
    "introducao": "SEC_02_INTRODUCAO.pdf",
    "por_pin": "SEC_03_INDICADORES.pdf",
    "encerramento": "SEC_10_ENCERRAMENTO.pdf"
}
...
PATH_PAGINAS_ALERTAS = {
    ...
    "tabelas_criticas_retro": "labels/8_5_1_pag.png",
    "tabelas_criticas_trator": "labels/8_5_2_pag.png",
    "tabelas_criticas_carre": "labels/8_5_3_pag.png",
    "tabelas_criticas_escav": "labels/8_5_4_pag.png",
    "tabelas_critica_moto": "labels/8_5_5_pag.png",
...
def func_hora_decimal_iso(valor):
    """Converte horas decimais para hh mm ss, preservando ausencia de dados em tabelas."""
    
    if pd.isna(valor):
        return MENSAGEM_SEM_DADOS_TABELA

    texto_original = str(valor).strip()
    if texto_original in {"", "nan", "NaN", "None", "SEM DADOS", "Sem Dados", "SEM_DADOS"}:
        return MENSAGEM_SEM_DADOS_TABELA
    ...
    if ":" in texto:
        ...
        td = pd.to_timedelta(numero, unit="h") if pd.notna(numero) else pd.NaT
    ...
    return f"{segundos // 3600:02d}h {(segundos % 3600) // 60:02d}m {segundos % 60:02d}s"
...
def func_formatacao_valor_tabela(valor, decimal: str = ".") -> str:
    """Padroniza valores ausentes, zerados ou NaN apenas para saida em tabelas."""
    ...
    if isinstance(valor, str):
        texto = valor.strip()
        if texto in {"", "nan", "NaN", "None", "SEM DADOS", "Sem Dados", "SEM_DADOS"}:
            return MENSAGEM_SEM_DADOS_TABELA
    ...
    return func_formatacao_numero(numero) if isinstance(valor, (int, float)) else str(valor)

def func_formatacao_saida_tabela(
    df: pd.DataFrame,
    decimal: str = ".",
) -> pd.DataFrame:
    """Cria uma copia propria para tabelas, sem alterar calculos ou validacoes."""
    ...
    for coluna in df_fmt.columns:
        df_fmt[coluna] = df_fmt[coluna].map(
            lambda valor: func_formatacao_valor_tabela(valor, decimal=decimal)
        )
...
def func_formatacao_decimal_horas(df):
    
    for coluna in ["horas_trabalhadas", "horas_ociosas"]:
        
        df[coluna] = df[coluna].apply(func_hora_decimal_iso)
...
def formatar_kpi(valor):
    ...
    return func_formatacao_numero(valor)
# ============================================================================
...
gara_cores = ["#C6EFCE", AMARELO_ALERTA, "#FFC7CE", "#FFC7CE"]

# === TABELA 6 - Formatação de rótulos: tabela de análise de óleos

fmt_tabela_7 = func_formatacao_tabelas(
    
    nomes_colunas={

        "numero_amostra": "Amostra",
        "chassi": "Chassi",
        "status": "Resultado",
        "data_finalizacao_amostra": "Data de Finalização",
        "nome_compartimento": "Compartimento",
        "acoes_inspecao": 'Ações',
        
        })

# === Formatação condicional - análise de óleo

QUIMICA_VALORES = ["NORMAL", "ANORMAL", "ATENCAO", "CRITICO"]
QUIMICA_CORES = ["#26DE3F", AMARELO_JOHN_DEERE, AMARELO_ALERTA, VERMELHO]

# === Colunas esperadas por consulta
# Mantem a estrutura dos DataFrames mesmo quando a consulta retorna sem linhas,
# evitando que os fluxos de metricas e salvamentos quebrem por ausência de coluna.

COLUNAS_CONSULTAS = {
    "frota": ["chassi", "estado", "cidade", "latitude", "longitude"],
    "horimetros": ["horimetro"],
    "comunicacao": ["chassi", "modelo", "data_comunicacao", "status_comunicacao"],
    "uso_diario": [
        "pin", "data_final", "work_hours", "idle_hours", "fuel_consumed",
        "model_clean", "fuel_rate_reference", "isg_type_name",
    ],
    "uso_periodo": [
        "pin", "isg_type_name", "model_clean", "work_hours", "idle_hours",
        "fuel_consumed", "fuel_rate", "fuel_rate_reference",
    ],
    "alertas": [
        "serial_number", "model_name", "type_name", "alert_data",
        "description", "severity",
    ],
    "analises_quimicas": [
        "numero_amostra", "chassi", "status", "data_finalizacao_amostra",
        "nome_compartimento", "acoes_inspecao",
    ],
    "garantia": [
        "pin", "status_garantia_basica", "dias_para_vencimento_basica",
        "data_vencimento_garantia_basica", "status_garantia_estendida",
        "dias_para_vencimento_estendida", "tipo_garantia_estendida",
...
def func_normalizar_dataframe_consulta(
    df: pd.DataFrame,
    colunas: list[str],
    valor_padrao=pd.NA,
) -> pd.DataFrame:
    """Padroniza consultas vazias para preservar as colunas esperadas."""

    if df is None or not isinstance(df, pd.DataFrame):
        df = pd.DataFrame()

    return func_validacao_colu(df, colunas, valor_padrao=valor_padrao)

def func_normalizar_consultas(consultas: dict[str, pd.DataFrame]) -> dict[str, pd.DataFrame]:
    """Aplica o contrato de colunas em todas as consultas do relatório."""

    consultas_normalizadas = {}

    for nome, df in consultas.items():
        consultas_normalizadas[nome] = func_normalizar_dataframe_consulta(
            df=df,
            colunas=COLUNAS_CONSULTAS.get(nome, []),
        )

    return consultas_normalizadas

def func_consulta_possui_registros(df: pd.DataFrame) -> bool:
    """Indica se a consulta retornou ao menos uma linha de dados."""

    return isinstance(df, pd.DataFrame) and not df.empty

def func_consulta_possui_dados_validos(df: pd.DataFrame, coluna_referencia: str | None = None) -> bool:
    """Valida se a consulta possui registros uteis para páginas de período."""

    if not func_consulta_possui_registros(df):
        return False

    if not coluna_referencia or coluna_referencia not in df.columns:
        return True

    serie = df[coluna_referencia].astype(str).str.strip()
    valores_sem_dados = {"", "nan", "None", "SEM DADOS", "Sem Dados", "SEM_DADOS"}
    return not serie.isin(valores_sem_dados).all()


def func_desempenho_modelo_possui_dados_validos(
    df: pd.DataFrame,
    colunas_validacao: list[str] | None = None,
) -> bool:
    """Valida se a seção de desempenho por modelo possui dados úteis.

    A seção deve ser ignorada quando todos os registros existentes estão sem
    dados nas colunas essenciais de utilização: horas trabalhadas, horas
    ociosas, eficiência e ociosidade. Valores nulos, vazios, textos de
    ausência de dados e zeros são tratados como ausência de dados, seguindo
    a mesma regra visual usada nas tabelas.
    """

    if not func_consulta_possui_registros(df):
        return False

    colunas_validacao = colunas_validacao or [
        "horas_trabalhadas",
        "horas_ociosas",
        "eficiencia_pct",
        "ociosidade_pct",
    ...
    colunas_existentes = [coluna for coluna in colunas_validacao if coluna in df.columns]

    if len(colunas_existentes) != len(colunas_validacao):
        return False
    ...
    df_validacao = df[colunas_existentes].copy()

    mascara_sem_dados = pd.DataFrame(index=df_validacao.index)

    for coluna in colunas_existentes:
        serie = df_validacao[coluna]
        ...
        mascara_sem_dados[coluna] = (
            serie.isna()
            | texto.isin(valores_sem_dados)
            | numero.eq(0)
        )

    linhas_totalmente_sem_dados = mascara_sem_dados.all(axis=1).sum()
    total_linhas = len(df_validacao)

    return linhas_totalmente_sem_dados != total_linhas
...
def func_validacao_valores_graficos(
    func_grafico,
    df: pd.DataFrame,
    mensagem: str = MENSAGEM_SEM_DADOS,
...
def func_output_tabela(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return func_formatacao_saida_tabela(df)

def func_output_pag_alertas(pdf: Relatorio, secoes: list[dict], fmt: dict) -> None:
    """Monta apenas as paginas A3 de alertas criticos que possuem dados."""

    secoes_com_dados = [
        secao
        for secao in secoes
        if func_consulta_possui_registros(secao.get("df"))
    ]

    total = len(secoes_com_dados)

    for indice, secao in enumerate(secoes_com_dados):
        df_alerta = secao["df"]
        ...
        pdf.add_dataframe(
            func_formatacao_saida_tabela(df_alerta),
            fmt=fmt,
            table_align="center",
            text_align="center",
        )
...
def func_output_pag_formato_a3(
    ...
    if df_tabela is not None and not df_tabela.empty:
        pdf.add_dataframe(func_formatacao_saida_tabela(df_tabela), fmt=fmt_tabela, table_align="center", text_align="center")
    else:
        pdf.add_paragraph(
            text=MENSAGEM_SEM_DADOS_TABELA,
            font_size=12,
            leading_cm=1,
            align="center",
        )
...
def func_cache_lotes(especificacoes: list[dict]) -> dict:
    ...
    for spec in especificacoes:
        ...
        resultados[spec["nome"]] = func_output_graficos(
            spec["func"],
            spec["df"],
            mensagem=spec.get("mensagem", MENSAGEM_SEM_DADOS),
...
def func_adicionar_tabela_comunicacao(
    ...
    pdf.add_dataframe(
        func_formatacao_saida_tabela(df),
        fmt=fmt,
        table_align="center",
        text_align="center",
        cond_col=cond_col,
...
def func_criar_pagina_sem_dados(
    nome_arquivo: str,
    formato_pagina=A4,
    ...
    """Gera uma seção com apenas uma página e a mensagem padrão de ausência de dados."""
    ...
    if background_image:
        pdf.set_background_image(background_image)

    pdf.spacer_cm(spacer_cm)
    pdf.add_paragraph(
        text=MENSAGEM_SEM_DADOS,
        font_size=font_size,
        leading_cm=1,
        align="center",
    )
...
def salvar_pdf_a3_desempenho(nome_arquivo: str, secoes: list[dict], fmt_tabela: dict) -> None:
    """Gera o PDF auxiliar A3 de desempenho apenas para modelos com dados."""

    secoes_com_dados = [
        secao
        for secao in secoes
        if func_desempenho_modelo_possui_dados_validos(
            secao.get("df_referencia", secao.get("df_tabela"))
        )
    ]

    if not secoes_com_dados:
        func_criar_pagina_sem_dados(
            nome_arquivo=nome_arquivo,
            formato_pagina=landscape(A3),
            margins=(1 * cm, 1 * cm, 1 * cm, 1 * cm),
            background_image=secoes[0].get("bg_tabela") if secoes else None,
            spacer_cm=12,
            font_size=30,
        )
        return

    secoes = secoes_com_dados
    ...
    if ultima["df_tabela"] is not None and not ultima["df_tabela"].empty:
        pdf.add_dataframe(func_formatacao_saida_tabela(ultima["df_tabela"]), fmt=fmt_tabela, table_align="center", text_align="center")
    else:
        pdf.spacer_cm(12)
        pdf.add_paragraph(text=MENSAGEM_SEM_DADOS_TABELA, font_size=12, leading_cm=1, align="center")
...
def func_output_pag_tabela_a3(
    ...
    pdf.add_dataframe(
        func_formatacao_saida_tabela(df_tabela),
        fmt=fmt_tabela,
        table_align="center",
        text_align="center",
        **kwargs_condicionais,
...
def salvar_pdf_a3_garantia(nome_arquivo: str, secoes: list[dict]) -> bool:
    """Gera o PDF auxiliar A3 da garantia apenas quando houver tabelas válidas.

    Cada seção de garantia é avaliada individualmente. Quando a garantia
    básica ou estendida não retorna registros vigentes, a respectiva página
    é ignorada. Se nenhuma das duas retornar dados, nenhum PDF é criado e a
    função retorna False para que a seção seja removida do compilado final.
    """

    secoes_com_dados = [
        secao
        for secao in secoes
        if func_consulta_possui_registros(secao.get("df_tabela"))
    ]
...
# --------------------------------------------------------------------
# GERAÇÃO DE RELATÓRIO - FLUXO UNIFICADO
# --------------------------------------------------------------------

def rda_py_gerar(cliente_id: int, data_inicial: str, data_final: str, datas_relatorio=None):

    # Etapa 0: valida recursos locais sem finalizar o interpretador.

    # Etapa inicial: dados de periodo usados em textos e nomes de arquivo.
    datas_relatorio = datas_relatorio or func_output_data()

    # ------------------------------------------------------------------
    # Consultas ao banco em paralelo (ThreadPoolExecutor)
    # Cada consulta é uma chamada de rede independente — rodar em paralelo
    # reduz o tempo total de ~8x sequencial para ~1x (limitado pela mais lenta).
    # ------------------------------------------------------------------
    from concurrent.futures import ThreadPoolExecutor, as_completed

    _consultas_def = {
        "nome_cliente": lambda: kernel_consulta.consultar_nome_cliente(id_client=cliente_id),
        "frota": lambda: kernel_consulta.consultar_geolocalizacao(id_client=cliente_id),
        "horimetros": lambda: kernel_consulta.consultar_horimetros(id_client=cliente_id),
        "comunicacao": lambda: kernel_consulta.consultar_status_comunicacao(id_client=cliente_id),
        "uso_diario": lambda: kernel_consulta.consultar_utilizacao_frota_diaria(
            id_client=cliente_id, data_inicial=data_inicial, data_final=data_final),
        "uso_periodo": lambda: kernel_consulta.consultar_utilizacao_frota_acumulada(
            id_client=cliente_id, data_inicial=data_inicial, data_final=data_final),
        "alertas": lambda: kernel_consulta.consultar_alertas(
            id_client=cliente_id, data_inicial=data_inicial, data_final=data_final),
        "analises_quimicas": lambda: kernel_consulta.analises_quimicas(
            id_client=cliente_id, data_inicial=data_inicial, data_final=data_final),
        "garantia": lambda: kernel_consulta.consultar_garantia(id_client=cliente_id),
    }

    _resultados = {}
    with ThreadPoolExecutor(max_workers=min(len(_consultas_def), 6)) as _executor:
        _futures = {_executor.submit(fn): nome for nome, fn in _consultas_def.items()}
        for _future in as_completed(_futures):
            _nome = _futures[_future]
            _resultados[_nome] = _future.result()

    _nome_cliente_df = _resultados["nome_cliente"]
    nome_cliente = str(_nome_cliente_df.iloc[0, 0]) if not _nome_cliente_df.empty else "Não Identificado"

    cliente_frota = _resultados["frota"]
    cliente_horimetros = _resultados["horimetros"]
    cliente_comunicacao = _resultados["comunicacao"]
    cliente_uso_diario = _resultados["uso_diario"]
    cliente_uso_periodo = _resultados["uso_periodo"]
    cliente_alertas = _resultados["alertas"]
    cliente_analises_quimicas = _resultados["analises_quimicas"]
    cliente_garantia = _resultados["garantia"]

    # Normaliza as consultas vazias, preservando as colunas usadas nas etapas seguintes.
    consultas = func_normalizar_consultas({
        "frota": cliente_frota,
        "horimetros": cliente_horimetros,
        "comunicacao": cliente_comunicacao,
        "uso_diario": cliente_uso_diario,
        "uso_periodo": cliente_uso_periodo,
        "alertas": cliente_alertas,
        "analises_quimicas": cliente_analises_quimicas,
        "garantia": cliente_garantia,
    })

    cliente_frota = consultas["frota"]
    cliente_horimetros = consultas["horimetros"]
    cliente_comunicacao = consultas["comunicacao"]
    cliente_uso_diario = consultas["uso_diario"]
    cliente_uso_periodo = consultas["uso_periodo"]
    cliente_alertas = consultas["alertas"]
    cliente_analises_quimicas = consultas["analises_quimicas"]
    cliente_garantia = consultas["garantia"]

    consulta_tem_registros = {
        "geolocalizacao": func_consulta_possui_registros(cliente_frota),
        "horimetros": func_consulta_possui_registros(cliente_horimetros),
        "comunicacao": func_consulta_possui_registros(cliente_comunicacao),
        "utilizacao": (
            func_consulta_possui_dados_validos(cliente_uso_diario, "work_hours")
            or func_consulta_possui_dados_validos(cliente_uso_periodo, "work_hours")
        ),
        "alertas": func_consulta_possui_registros(cliente_alertas),
        "analises_quimicas": func_consulta_possui_registros(cliente_analises_quimicas),
        "garantia": func_consulta_possui_registros(cliente_garantia),
    }
    ...
    df_resumo_retro_tbl = func_formatacao_decimal_horas(df_resumo_retro_tbl)
    df_resumo_trator_tbl = func_formatacao_decimal_horas(df_resumo_trator_tbl)
    df_resumo_moto_tbl = func_formatacao_decimal_horas(df_resumo_moto_tbl)
    df_resumo_carrega_tbl = func_formatacao_decimal_horas(df_resumo_carrega_tbl)
    df_resumo_escava_tbl = func_formatacao_decimal_horas(df_resumo_escava_tbl)

    #Métricas de Envelhecimento da Frota
    
    idade_media = round(cliente_horimetros["horimetro"].mean(), 2) if not cliente_horimetros.empty else 0
    ...
    .value_counts().reindex(["Alertas Informativos", "Alertas de Atenção", "Alertas Criticos"])
    ...
    sum_alertas_medium = (
    contagem_alertas_criticidade.loc[
        contagem_alertas_criticidade["severity"] == "Alertas de Atenção",
        "quantidade"
    ]
    .sum())
    ...
    alertas_por_familia[["perc_info", "perc_aten", "perc_critico"]] = (
        alertas_por_familia[
            ["Alertas Informativos", "Alertas de Atenção", "Alertas Criticos"]
        ]
    ...
    total_amostras_criticas = quimica_frota.loc[quimica_frota["status"] == "CRITICO", "numero_amostra"].count()
    total_amostras_atencao = quimica_frota.loc[quimica_frota["status"] == "ATENCAO", "numero_amostra"].count()
    total_amostras_anormal = quimica_frota.loc[quimica_frota["status"] == "ANORMAL", "numero_amostra"].count()
    total_amostras_normal = quimica_frota.loc[quimica_frota["status"] == "NORMAL", "numero_amostra"].count()
    ...
    graficos = func_cache_lotes([
        ...
        },
        {
            ...
            "kwargs": {
                ...
                "cores": {
                    ...
                    "Alertas de Atenção": {"quantidade": AMARELO_ALERTA},
        ...
        },
        {
            ...
            "colunas_validacao": ["Alertas Criticos", "Alertas Informativos", "Alertas de Atenção"],
            "kwargs": {
                "coluna_categoria": "type_name",
                "colunas_valores": ["Alertas Criticos", "Alertas Informativos", "Alertas de Atenção"],
                "titulo": " ",
                "orientacao": "v",
        ...
        },
        {
            ...
            "func": kernel_graficos.grafico_barras_verticais,
            "df": alertas_por_dia,
            "colunas_validacao": ["Alertas de Atenção"],
            "kwargs": {
                "coluna_categoria": "alert_data",
                "coluna_valor": "Alertas de Atenção",
                "titulo": " ",
    ...
    for nome_familia, df_familia, arquivo_ociosidade, arquivo_consumo in familias_performance:
        if not func_desempenho_modelo_possui_dados_validos(df_familia):
            continue
    ...
    retro_1 = graficos.get("retro_1", {"tipo": "texto", "conteudo": MENSAGEM_SEM_DADOS})
    retro_2 = graficos.get("retro_2", {"tipo": "texto", "conteudo": MENSAGEM_SEM_DADOS})
    trator_1 = graficos.get("trator_1", {"tipo": "texto", "conteudo": MENSAGEM_SEM_DADOS})
    trator_2 = graficos.get("trator_2", {"tipo": "texto", "conteudo": MENSAGEM_SEM_DADOS})
    escava_1 = graficos.get("escava_1", {"tipo": "texto", "conteudo": MENSAGEM_SEM_DADOS})
    escava_2 = graficos.get("escava_2", {"tipo": "texto", "conteudo": MENSAGEM_SEM_DADOS})
    moto_1 = graficos.get("moto_1", {"tipo": "texto", "conteudo": MENSAGEM_SEM_DADOS})
    moto_2 = graficos.get("moto_2", {"tipo": "texto", "conteudo": MENSAGEM_SEM_DADOS})
    carreg_1 = graficos.get("carreg_1", {"tipo": "texto", "conteudo": MENSAGEM_SEM_DADOS})
    carreg_2 = graficos.get("carreg_2", {"tipo": "texto", "conteudo": MENSAGEM_SEM_DADOS})
   ...
   # ------------------------------------------------------------------------
    ...
    if consulta_tem_registros["utilizacao"]:
        secoes_a3_desempenho = [
            ...
            },
            {
                ...
                "df_referencia": df_resumo_escava,
                "figura_1": escava_1,
                "figura_2": escava_2,
            },
            {
                ...
                "df_referencia": df_resumo_trator,
                "figura_1": trator_1,
                "figura_2": trator_2,
            },
            {
            ...
            },
            {
                ...
                "df_referencia": df_resumo_carrega,
                "figura_1": carreg_1,
                "figura_2": carreg_2,
            },
        ]
    ...
    # Se a consulta de alertas nao retornar registros, grava apenas uma pagina
    # com a mensagem padrao, sem gerar paginas de graficos e tabelas vazias.
    # ------------------------------------------------------------------------

    if consulta_tem_registros["alertas"]:
        ...
        secoes_alertas = [
            {"df": alertas_criticos_retro, "bg": PATH_PAGINAS_ALERTAS["tabelas_criticas_retro"]},
            {"df": alertas_criticos_trator, "bg": PATH_PAGINAS_ALERTAS["tabelas_criticas_trator"]},
            {"df": alertas_criticos_moto, "bg": PATH_PAGINAS_ALERTAS["tabelas_critica_moto"]},
            {"df": alertas_criticos_escav, "bg": PATH_PAGINAS_ALERTAS["tabelas_criticas_escav"]},
            {"df": alertas_criticos_carreg, "bg": PATH_PAGINAS_ALERTAS["tabelas_criticas_carre"]},
        ]
        secoes_alertas_com_dados = [
            secao for secao in secoes_alertas
            if func_consulta_possui_registros(secao.get("df"))
        ]

        if secoes_alertas_com_dados:
            secoes_a3_alertas.new_page()
            func_output_pag_alertas(secoes_a3_alertas, secoes_alertas_com_dados, fmt_tabela_4)
        ...
        func_criar_pagina_sem_dados(
            ...
            formato_pagina=landscape(A3),
            margins=(1 * cm, 1 * cm, 1 * cm, 1 * cm),
            background_image=PATH_PAGINAS_ALERTAS["grafico_1"],
            spacer_cm=12,
            font_size=30,
        )

    #===================================================================
    # PDF Auxiliar de Amostras de Óleo em Caso de Resultados Existentes
    # Segue o padrão if/else: com registros, gera tabela; sem registros,
    # grava apenas uma página com a mensagem solicitada.
    #===================================================================

    arquivo_analises_oleo = "AMOSTRAS_DE_OLEO.pdf"

    if amostras_periodo > 0:
        amostrasresults = Relatorio(
            nome_arquivo=f"A3_AMOSTRAS_DE_OLEO.pdf",
            formato_pagina=landscape(A3),
            margins=(1 * cm, 1 * cm, 1 * cm, 1 * cm),
        )
        amostrasresults.set_background_image("labels/9_1_y_pag.png")
        amostrasresults.spacer_cm(10)
        cliente_analises_quimicas["acoes_inspecao"] = cliente_analises_quimicas["acoes_inspecao"].fillna(
            "Sem Ações Recomendas"
        )

        amostrasresults.new_add_dataframe(
            func_formatacao_saida_tabela(cliente_analises_quimicas),
            fmt=fmt_tabela_7,
            table_align="center",
            text_align="center",
            cond_col="status",
            cond_values=QUIMICA_VALORES,
            cond_colors=QUIMICA_CORES,
        )
        amostrasresults.save()

        amostras = Relatorio(
            nome_arquivo=arquivo_analises_oleo,
            formato_pagina=A4,
            margins=(3 * cm, 2 * cm, 2 * cm, 2 * cm),
        )
        amostras.set_background_image("labels/9_1_pag.png")
        amostras.insert_external_pdf(external_pdf_path="A3_AMOSTRAS_DE_OLEO.pdf", insert_at_page=2)
        ...
        func_criar_pagina_sem_dados(
            ...
            formato_pagina=A4,
            margins=(3 * cm, 2 * cm, 2 * cm, 2 * cm),
            background_image=PATH_PAGINAS_COMPLEMENTARES["amostras_sem_resultado"],
            spacer_cm=10,
            font_size=24,
        )
    ...
    arquivo_principal = f"relatorio_{cliente_id}_{data_inicial}_{data_final}.pdf"
    ...
    pdf_indicadores.spacer_cm(0.2)
    pdf_indicadores.kpi_cards(
        titulos=["Total de Alertas", "Alertas de Atenção", "Alertas Criticos"],
        valores=[
    ...
    pdf_indicadores.kpi_cards(
        ...
        cores_valor=[PRETO_JOHN_DEERE, PRETO_JOHN_DEERE, BRANCO],
        cores_titulo=[PRETO_JOHN_DEERE, PRETO_JOHN_DEERE, BRANCO],
        cores_borda=[AMARELO_JOHN_DEERE, "#26DE3F", VERMELHO],
        cores_fundo=[AMARELO_JOHN_DEERE, "#26DE3F", VERMELHO],
        cores_rotulo=[CINZA_JOHN_DEERE, PRETO_JOHN_DEERE, BRANCO],
        fmt=KPI_FMT,
        espacamento_cm=0.2,
        y_cm=13.5,
    )
    ...
    if consulta_tem_registros["comunicacao"]:
        ...
        func_criar_pagina_sem_dados(
            nome_arquivo=ARQUIVOS_SECOES_PDF["comunicacao"],
            formato_pagina=A4,
            margins=(3 * cm, 2 * cm, 2 * cm, 2 * cm),
            background_image="labels/5_pag.png",
            spacer_cm=10,
            font_size=24,
        )
    ...
    if consulta_tem_registros["geolocalizacao"]:
        ...
        pdf_geo.add_dataframe(
            func_formatacao_saida_tabela(geolocalizacao[["chassi", "estado", "cidade"]]),
            fmt=fmt_tabela_2,
            table_align="center",
            text_align="center",
        )
        ...
        func_criar_pagina_sem_dados(
            nome_arquivo=ARQUIVOS_SECOES_PDF["geolocalizacao"],
            formato_pagina=A4,
            margins=(3 * cm, 2 * cm, 2 * cm, 2 * cm),
            background_image="labels/6_pag.png",
            spacer_cm=10,
            font_size=24,
        )
    ...
    if consulta_tem_registros["utilizacao"]:
        ...
        valores=[
            formatar_kpi(consumo_retro),
            formatar_kpi(consumo_moto),
            formatar_kpi(consumo_trator),
            formatar_kpi(consumo_esca),
            formatar_kpi(consumo_carreg),
        ],
        rotulos=["Média em Litros por Hora"] * 5,
        cores_valor=[PRETO_JOHN_DEERE] * 5,
        cores_titulo=[PRETO_JOHN_DEERE] * 5,
        cores_borda=[AMARELO_JOHN_DEERE] * 5,
        cores_fundo=[AMARELO_JOHN_DEERE] * 5,
        ...
        func_criar_pagina_sem_dados(
            nome_arquivo=ARQUIVOS_SECOES_PDF["utilizacao_resumo"],
            formato_pagina=A4,
            margins=(3 * cm, 2 * cm, 2 * cm, 2 * cm),
            background_image="labels/7_0_pag.png",
            spacer_cm=10,
            font_size=24,
        )
    ...
    if consulta_tem_registros["alertas"]:
        ...
        func_criar_pagina_sem_dados(
            nome_arquivo=ARQUIVOS_SECOES_PDF["alertas_abertura"],
            formato_pagina=A4,
            margins=(3 * cm, 2 * cm, 2 * cm, 2 * cm),
            background_image="labels/8_0_pag.png",
            spacer_cm=10,
            font_size=24,
        )
    ...
    # --------------------------------------------------------------------
    # O arquivo da seção já foi definido no fluxo condicional de amostras.
    ...
    df_garantia_estendida = df_garantia_estendida[
    df_garantia_estendida['status_garantia_estendida']
    .fillna('')
    .astype(str)
    .str.strip()
    .eq('VIGENTE')]

    df_garantia_basica = df_garantia_basica[
    df_garantia_basica['status_garantia_basica']
    .fillna('')
    .astype(str)
    .str.strip()
    .eq('VIGENTE')]
    ...
    garantia_tem_tabelas_vigentes = any(
        func_consulta_possui_registros(secao.get("df_tabela"))
        for secao in secoes_a3_garantia
    )

    garantia_a3_gerada = False
    if garantia_tem_tabelas_vigentes:
        garantia_a3_gerada = salvar_pdf_a3_garantia(arquivo_a3_garantia, secoes_a3_garantia)


    if consulta_tem_registros["garantia"] and garantia_tem_tabelas_vigentes:
    ...
    # Se nenhuma tabela de garantia vigente existir, a seção de garantia
    # não é criada e o compilado final segue diretamente para a próxima seção.

    # --------------------------------------------------------------------
    # SECAO 10 - ENCERRAMENTO
    # --------------------------------------------------------------------
    arquivo_pre_encerramento = "labels/11_1_pag.pdf"
    ...
    secoes_compilado_final = [
        ...
        *(
            ...
            if garantia_tem_tabelas_vigentes and garantia_a3_gerada
            else []
        ),
        {"ordem": 13, "nome": "preencerramento", "arquivo": arquivo_pre_encerramento},
        {"ordem": 14, "nome": "encerramento", "arquivo": arquivo_encerramento},
    ]
...
# --------------------------------------------------------------------
# GERAÇÃO DE RELATÓRIO - DATA FIXA
# --------------------------------------------------------------------

def rda_py_data_fixa(cliente_id: int):

    """Executa o relatório usando a janela padrão dos últimos 30 dias."""
...
def rda_py_data_movel(cliente_id: int, data_inicial: str, data_final: str):
    """Executa o relatório usando o período informado pelo usuário."""

    return rda_py_gerar(
        cliente_id=cliente_id,
        data_inicial=data_inicial,
        data_final=data_final,
        datas_relatorio=func_output_data(),
    )

# --------------------------------------------------------------------
# GERAÇÃO DE RELATÓRIO - POR CHASSI - DATA MÓVEL (NOVA FUNÇÃO - 18/05/2026 - thiago.barros)
# --------------------------------------------------------------------

def rda_py_by_pin(chassi: str, data_inicial: str, data_final: str):

    # ------------------------------------------------------------------
    # Consultas ao banco em paralelo (mesmo padrão de rda_py_gerar)
    # ------------------------------------------------------------------
    from concurrent.futures import ThreadPoolExecutor, as_completed

    _pin_consultas = {
        "nome_cliente": lambda: kernel_consulta.consultar_nome_cliente_by_pin(pin=chassi),
        "geolocalizacao": lambda: kernel_consulta.consultar_geolocalizacao_by_pin(pin=chassi),
        "horimetro": lambda: kernel_consulta.consultar_horimetros_by_pin(pin=chassi),
        "comunicacao": lambda: kernel_consulta.consultar_status_comunicacao_by_pin(pin=chassi),
        "uso_diario": lambda: kernel_consulta.consultar_utilizacao_frota_diaria_by_pin(
            pin=chassi, data_inicial=data_inicial, data_final=data_final),
        "uso_periodo": lambda: kernel_consulta.consultar_utilizacao_frota_acumulada_by_pin(
            pin=chassi, data_inicial=data_inicial, data_final=data_final),
        "alertas": lambda: kernel_consulta.consultar_alertas_by_pin(
            pin=chassi, data_inicial=data_inicial, data_final=data_final),
        "analise_quimica": lambda: kernel_consulta.consultar_analises_quimicas_by_pin(
            pin=chassi, data_inicial=data_inicial, data_final=data_final),
        "garantia": lambda: kernel_consulta.consultar_garantia_by_pin(pin=chassi),
    }

    _pin_res = {}
    with ThreadPoolExecutor(max_workers=min(len(_pin_consultas), 6)) as _executor:
        _futures = {_executor.submit(fn): nome for nome, fn in _pin_consultas.items()}
        for _future in as_completed(_futures):
            _nome = _futures[_future]
            _pin_res[_nome] = _future.result()

    nome_cliente_pin = _pin_res["nome_cliente"]
    geolocalizacao_pin = _pin_res["geolocalizacao"]
    geolocalizacao_pin['cidade'] = geolocalizacao_pin['cidade'].str.split('-').str[1]
    horimetro_pin = _pin_res["horimetro"]
    comunicacao_pin = _pin_res["comunicacao"]
    uso_diario_pin = _pin_res["uso_diario"]
    uso_periodo_pin = _pin_res["uso_periodo"]
    total_alertas_pin = _pin_res["alertas"]
    analise_quimica_pin = _pin_res["analise_quimica"]
    garantia_pin = _pin_res["garantia"]

    # ETAPA 1 : Normalização de Consultas Vazias

    if not uso_periodo_pin.empty:

        tipo_maquina = uso_periodo_pin['isg_type_name'].iloc[0]

        mapa_maquinas = {
            'Excavator': ('icons/machines/excavator.png', 'Escavadeira'),
            'Backhoe': ('icons/machines/backhoe.png', 'Retroescavadeira'),
            'Motor Grader': ('icons/machines/grader.png', 'Motoniveladora'),
            'Loader': ('icons/machines/loader.png', 'Carregadeira'),
            'Dozer': ('icons/machines/grader.png', 'Trator de Esteiras'),
        }

        figura_modelo, familia_maquina = mapa_maquinas.get(
            tipo_maquina,
        ...
        figura_modelo = None
        familia_maquina = 'Não Identificada'

    ordem_criticidade = ["INFO", "MEDIUM", "HIGH", "CRITICAL"]

    severity_limpa = (
        total_alertas_pin["severity"]
        .fillna("")
        .astype(str)
    ...
    contagem_alertas_criticidade = (
        severity_limpa
        .value_counts()
        .reindex(ordem_criticidade, fill_value=0)
    ...
    valores_criticidade = (
        contagem_alertas_criticidade
        .set_index("severity")["quantidade"]
        .to_dict()
    )

    #cabeçalho
    nome_cliente = str(nome_cliente_pin.iloc[0, 0])
    modelo_equipamento = str(geolocalizacao_pin['modelo'].iloc[0]) 
    chassi = str(geolocalizacao_pin['chassi'].iloc[0])


    #revisão
    horimetro_atual = int(horimetro_pin['horimetro'].iloc[0])
    status_horimetro = str(horimetro_pin['status_horimetro'].iloc[0])
    aptidao_revisao = str(horimetro_pin['aptidao_revisao'].iloc[0])
    proxima_revisao = int(horimetro_pin['proxima_verificacao'].iloc[0])

    #geolocalização
    estado = str(geolocalizacao_pin['estado'].iloc[0])
    cidade = str(geolocalizacao_pin['cidade'].iloc[0])
    bandeira_estado = 'icons/flags/PE.png'

    #uso do equipamento

    work_hours = pd.to_numeric(
        uso_periodo_pin['work_hours'].iloc[0],
        errors='coerce'
    )

    idle_hours = pd.to_numeric(
        uso_periodo_pin['idle_hours'].iloc[0],
        errors='coerce'
    )

    # Trata NaN
    work_hours = 0 if pd.isna(work_hours) else work_hours
    idle_hours = 0 if pd.isna(idle_hours) else idle_hours

    # Totais
    total_de_horas_trabalhando = round(work_hours, 2)
    total_de_horas_ociosa = round(idle_hours, 2)

    total_horas_ligadas = round(
        total_de_horas_trabalhando + total_de_horas_ociosa,
        2
    )

    # Percentuais
    percentual_trabalhando = (
        round(total_de_horas_trabalhando / total_horas_ligadas, 2)
        if total_horas_ligadas > 0
        else 0
    )

    percentual_ocioso = (
        round(total_de_horas_ociosa / total_horas_ligadas, 2)
        if total_horas_ligadas > 0
        else 0
    )
    ...
    fuel_consumed = pd.to_numeric(
        uso_periodo_pin['fuel_consumed'].iloc[0],
        errors='coerce'
    )

    fuel_rate = pd.to_numeric(
        uso_periodo_pin['fuel_rate'].iloc[0],
        errors='coerce'
    )

    total_de_litros_consumido = round(fuel_consumed, 2) if fuel_consumed > 0 else 0
    taxa_de_consumo = round(fuel_rate, 2) if fuel_rate > 0 else 0
    ...
    total_de_kg_co2e_gerados = (
        round(fuel_rate * 12.49, 2)
        if fuel_rate > 0
        else 0
    )

    taxa_de_emissoes = (
        round(fuel_consumed * 0.77, 2)
        if fuel_consumed > 0
        else 0
    )
    ...
    alertas_info = valores_criticidade.get("INFO", 0)
    alertas_medium = valores_criticidade.get("MEDIUM", 0)
    alertas_high = valores_criticidade.get("HIGH", 0)
    alertas_critical = valores_criticidade.get("CRITICAL", 0)
    total_de_alertas = alertas_info + alertas_medium +  alertas_high + alertas_critical

    #alertas

    ordem_criticidade_amostras = [
        "NORMAL",
        "ANORMAL",
        "ATENCAO",
        "CRITICO"
    ...
    contagem_amostras_criticidade = (
        severidade_amostras
        .value_counts()
        .reindex(ordem_criticidade_amostras, fill_value=0)
        .rename_axis("status")
        .reset_index(name="quantidade")
    )
    ...
    valores_amostras = (
        contagem_amostras_criticidade
        .set_index("status")["quantidade"]
        .to_dict()
    )

    amostras_normal = valores_amostras.get("NORMAL", 0)
    amostras_anormal = valores_amostras.get("ANORMAL", 0)
    amostras_atencao = valores_amostras.get("ATENCAO", 0)
    amostras_critica = valores_amostras.get("CRITICO", 0)

    #garantia

    if not garantia_pin.empty:
        valor_garantia = garantia_pin["status_garantia_basica"].iloc[0]

        garantia_basica = (
            valor_garantia
            if pd.notna(valor_garantia) and str(valor_garantia).strip() != ""
            else "NÃO IDENTIFICADA"
        )
    else:
        garantia_basica = "NÃO IDENTIFICADA"

    # --------------------------------------------------------------------
    # ARQUIVO PRINCIPAL
    # --------------------------------------------------------------------

    arquivo_principal = f"relatorio_{chassi}_{data_inicial}_{data_final}.pdf"

    # --------------------------------------------------------------------
    # SECAO 01 - CAPA
    # --------------------------------------------------------------------

    pdf_capa = func_criar_pdf_a4(ARQUIVOS_SECOES_PDF_BY_PIN["capa"])
    pdf_capa.set_background_image("labels/1_capa.png")
    func_salvar_secao_pdf(pdf_capa)
    ...
    pdf_intro = func_criar_pdf_a4(ARQUIVOS_SECOES_PDF_BY_PIN["introducao"])
    pdf_intro.set_background_image("labels/2_intro.png")
    pdf_intro.spacer_cm(3)
    pdf_intro.add_paragraph(text=TEX1pin + "   " + nome_cliente + ".", font_size=12, leading_cm=1, align="justify")
    pdf_intro.spacer_cm(0.2)
    pdf_intro.add_paragraph(text=TEX2pin, font_size=12, leading_cm=1, align="justify")
    pdf_intro.spacer_cm(0.2)

    paragrafo_periodo = (
        f"O relatorio que voce esta recebendo abrange o periodo que vai de "
        f"{data_inicial} ate {data_final}, sendo referente ao chassi {chassi}."
    )

    pdf_intro.add_paragraph(text=TEX3pin + " " + paragrafo_periodo, font_size=12, leading_cm=1, align="justify")
    pdf_intro.spacer_cm(0.2)
    pdf_intro.add_paragraph(text=TEX4pin, font_size=12, leading_cm=1, align="justify")
    pdf_intro.spacer_cm(0.2)
    func_salvar_secao_pdf(pdf_intro)

    # --------------------------------------------------------------------
    # SECAO 03 - RELATÓRIO DE PERFOMANCE - POR CHASSI
    # --------------------------------------------------------------------

    #O sistema de coordenada é cartesiano com o centro 0,0 na ponta esquerda superior da folha. Esqueci de metrificar isso em cm e acabei deixando em pixels (foi mal).

    pdf_pin_principal = func_criar_pdf_a4(ARQUIVOS_SECOES_PDF_BY_PIN["por_pin"])
    pdf_pin_principal.set_background_image("labels/by_pin.png")

    #ELEMENTO 1: NOME DO CLIENTE

    x_offset_padrao = 146
    x_offset_padrao_segunda_coluna = 380 + 45


    pdf_pin_principal.add_text_move(

        text = f'{nome_cliente}',
        x_offset = x_offset_padrao,
        y_offset= 172,
        size = 12

    )

    #ELEMENTO 2: MODELO DO EQUIPAMENTO

    pdf_pin_principal.add_text_move(

        text = f'{modelo_equipamento}',
        x_offset = x_offset_padrao,
        y_offset= 202,
        size = 12
    ...
    #ELEMENTO 3: FAMÍLIA DO EQUIPAMENTO

    pdf_pin_principal.add_text_move(

        text = f'{familia_maquina}',
        x_offset = x_offset_padrao,
        y_offset= 232,
        size = 12
    ...
    #ELEMENTO 5: FIGURA DO MODELO

    pdf_pin_principal.add_image(

        image_path = figura_modelo,
        width = 155, 
        height = 110,
        x_offset = 420,
        y_offset= 157,
    ...
    #ELEMENTO 6: HORIMETRO ATUAL DO EQUIPAMENTO

    pdf_pin_principal.add_text_move(

        text = f'{horimetro_atual}',
        x_offset = x_offset_padrao,
        y_offset= 333,
        size = 12
    ...
    #ELEMENTO 7: STATUS DO HORÍMETRO

    pdf_pin_principal.add_text_move(

        text = f'{status_horimetro}',
        x_offset = x_offset_padrao,
        y_offset= 363,
        size = 12
    ...
    #ELEMENTO 8: APTO PARA REVISÃO

    pdf_pin_principal.add_text_move(

        text = f'{aptidao_revisao}',
        x_offset = x_offset_padrao,
        y_offset= 393,
        size = 12
    ...
    #ELEMENTO 9: PRÓXIMA REVISÃO

    pdf_pin_principal.add_text_move(

        text = f'{proxima_revisao}',
        x_offset = x_offset_padrao,
        y_offset= 423,
        size = 12
    ...
    #ELEMENTO 12: HORAS LIGADAS

    pdf_pin_principal.add_text_move(

        text = f'{func_hora_decimal_iso(total_horas_ligadas)}',
        x_offset = x_offset_padrao,
        y_offset= 490,
        size = 12
    ...
    #ELEMENTO 13: HORAS TRABALHADAS

    pdf_pin_principal.add_text_move(

        text = f'{func_hora_decimal_iso(total_de_horas_trabalhando)}',
        x_offset = x_offset_padrao,
        y_offset= 520,
        size = 12
    ...
    #ELEMENTO 14: HORAS OCIOSAS

    pdf_pin_principal.add_text_move(

        text = f'{func_hora_decimal_iso(total_de_horas_ociosa)}',
        x_offset = x_offset_padrao,
        y_offset= 550,
        size = 12
    ...
    #ELEMENTO 14: TOTAL DE ALERTAS

    pdf_pin_principal.add_text_move(

        text = f'{total_de_alertas}',
        x_offset = x_offset_padrao,
        y_offset= 618,
        size = 12
    )

    #ELEMENTO 15: TOTAL DE ALERTAS INFO
    ...
    #ELEMENTO 16: TOTAL DE ALERTAS MEDIOS

    pdf_pin_principal.add_text_move(

        text = f'{alertas_medium}',
        x_offset = x_offset_padrao,
        y_offset= 678,
        size = 12
    ...
    #ELEMENTO 17: TOTAL DE ALERTAS CRITICOS

    pdf_pin_principal.add_text_move(

        text = f'{alertas_critical + alertas_high}',
        x_offset = x_offset_padrao,
        y_offset= 708,
        size = 12
    ...
    #ELEMENTO 18: GARANTIA

    pdf_pin_principal.add_text_move(

        text = f'{garantia_basica}',
        x_offset = x_offset_padrao,
        y_offset= 772,
        size = 12
    ...
    pdf_pin_principal.add_text_move(
        ...
        text = f'{estado}',
        x_offset = x_offset_padrao_segunda_coluna - 80,
        y_offset= 335,
        size = 12
    ...
    pdf_pin_principal.add_text_move(
        ...
        text = f'{cidade}',
        x_offset = x_offset_padrao_segunda_coluna + 65,
        y_offset= 335,
        size = 12
    ...
    #ELEMENTO 19: TOTAL DE LITROS COMBUSTÍVEL

    pdf_pin_principal.add_text_move(

        text = f'{total_de_litros_consumido}',
        x_offset = x_offset_padrao_segunda_coluna,
        y_offset= 400,
        size = 12

    )
    
    #ELEMENTO 20: TAXAS DE CONSUMO

    pdf_pin_principal.add_text_move(

        text = f'{taxa_de_consumo}',
        x_offset = x_offset_padrao_segunda_coluna,
        y_offset= 430,
        size = 12
    ...
    #ELEMENTO 21: MASSA DE EMISSÕES GERADAS

    pdf_pin_principal.add_text_move(

        text = f'{total_de_kg_co2e_gerados}',
        x_offset = x_offset_padrao_segunda_coluna,
        y_offset= 495,
        size = 12
    )

    #ELEMENTO 22: TAXA DE EMISSÕES

    pdf_pin_principal.add_text_move(

        text = f'{taxa_de_emissoes}',
        x_offset = x_offset_padrao_segunda_coluna,
        y_offset= 530,
        size = 12
    ...
    #ELEMENTO 23: AMOSTRAS NORMAIS

    pdf_pin_principal.add_text_move(

        text = f'{amostras_normal}',
        x_offset = x_offset_padrao_segunda_coluna,
        y_offset= 590,
        size = 12
    ...
    #ELEMENTO 24: AMOSTRAS ANORMAIS

    pdf_pin_principal.add_text_move(
        text = f'{amostras_anormal}',
        x_offset = x_offset_padrao_segunda_coluna,
        y_offset= 620,
        size = 12
    )

    #ELEMENTO 25: AMOSTRAS DE ATENÇÃO

    pdf_pin_principal.add_text_move(

        text = f'{amostras_atencao}',
        x_offset = x_offset_padrao_segunda_coluna,
        y_offset= 650,
        size = 12
    )

    #ELEMENTO 26: AMOSTRAS CRÍTICAS

    pdf_pin_principal.add_text_move(

        text = f'{amostras_critica}',
        x_offset = x_offset_padrao_segunda_coluna,
        y_offset= 680,
        size = 12
    )

    func_salvar_secao_pdf(pdf_pin_principal)

    # --------------------------------------------------------------------
    # SECAO FINAL - COMPILAR PDF
    # --------------------------------------------------------------------
    ...
    secoes_compilado_final = [
        {"ordem": 1, "nome": "capa", "arquivo": ARQUIVOS_SECOES_PDF_BY_PIN["capa"]},
        {"ordem": 2, "nome": "introducao", "arquivo": ARQUIVOS_SECOES_PDF_BY_PIN["introducao"]},
        {"ordem": 3, "nome": "por_pin", "arquivo": ARQUIVOS_SECOES_PDF_BY_PIN["por_pin"]},
        {"ordem": 4, "nome": "preencerramento", "arquivo": arquivo_pre_encerramento},
        {"ordem": 5, "nome": "encerramento", "arquivo": arquivo_encerramento},
    ...
    func_compilar_pdfs(arquivo_principal, arquivos_compilado)
    artefatos_temporarios = [
        *ARQUIVOS_SECOES_PDF_BY_PIN.values(),
    ]
    func_cache_temporarios(artefatos_temporarios)
...
if __name__ == "__main__":
    ...
    print(YELLOW + r"""
                        ...
                        ██╔══██╗██║  ██║██╔══██║
                        ██║  ██║██████╔╝██║  ██║
        |RDA - RELATÓRIOS DE DESEMPENHO AUTOMÁTICOS - Versão 0.4 - 16/06/2026 - Versão Estável
        |VENEZA EQUIPAMENTOS PESADOS - JOHN DEERE
        |CENTRO DE SOLUÇÕES CONECTADAS - CSC
        |DESENVOLVIDO POR THIAGO BARROS - thiago.barros@venezanet.com
        """ + RESET)

    # Valida recursos sem usar sys.exit. Caso falhe, o menu permanece ativo.
    try:
        #vou adicionar, uma verificação depois
        pass
    ...
    while True:
        ...
        print("[3] - Fluxo Fixo - Toda a Lista de Clientes Prioritários")
        print("[4] - Data Móvel - Por Chassi")
        print("[5] - Encerrar a Aplicação")
        print("    ")

        try:
            flow = int(input("Entre Com o Tipo de Execução: ").strip())
        except ValueError:
            print("Valor inválido. Informe apenas 1, 2, 3, 5 ou 4.")
            continue
        ...
        if flow == 1:
            ...
            try:
                id_cliente = int(input("Insira o Código do Cliente: ").strip())
                arquivo_gerado = rda_py_data_fixa(id_cliente)
                print(f"Relatorio final gerado: {arquivo_gerado}")
                ...
                artefatos_temporarios = [
                    ...
                    *PATH_ALERTAS_IMAGENS.values(),
                    *PATH_PERFORMANCE_IMAGENS.values(),
                    "AMOSTRAS_DE_OLEO.pdf",
                    "A3_AMOSTRAS_DE_OLEO.pdf",
                ]
                func_cache_temporarios(artefatos_temporarios)
                continue
            ...
            try:
                id_cliente = int(input("Insira o Código do Cliente: ").strip())
                data_inicial = str(input("Insira a Data Inicial da Consulta: ")).strip()
                data_final = str(input("Insira a Data Final da Consulta: ")).strip()
                arquivo_gerado = rda_py_data_movel(id_cliente, data_inicial, data_final)
                print(f"Relatorio final gerado: {arquivo_gerado}")
            ...
            for id_client in tqdm(valores, desc=f"Processando Relatórios:"):

                try:
                    arquivo_gerado = rda_py_data_fixa(id_client)
            ...
            try:
                pin = str(input("Insira o Chassi: ").strip())
                data_inicial = str(input("Insira a Data Inicial da Consulta: ")).strip()
                data_final = str(input("Insira a Data Final da Consulta: ")).strip()
                arquivo_gerado = rda_py_by_pin(pin, data_inicial, data_final)
                print(f"Relatorio final gerado: {arquivo_gerado}")
            ...
            print("Encerrando o Fluxo")
            break

        elif flow == 96:
            print('Modo Teste 1')
            arquivo_gerado = rda_py_by_pin('1BZ524PAPRDX00048', '01-05-2026', '04-05-2026')
            print(f"Relatorio final gerado: {arquivo_gerado}")

        elif flow == 87:
            print('Modo Teste 2')
            arquivo_gerado = rda_py_data_movel(5775022, '01-05-2026', '04-05-2026')
            print(f"Relatorio final gerado: {arquivo_gerado}")


        else:
            print("Valor Não Identificado. Informe apenas 1, 2, 3 ou 4.")
            continue
