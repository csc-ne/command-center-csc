...
# VERSÃO ESTÁVEL - 0.3 - Data 26/05/2026 - Adição de página de verificação dos analistas
# adição dos novos layouts.
# =======================================================================
...
# === Fonte Helvetica (registrada a partir dos TTFs incluídos no projeto)

import matplotlib.font_manager as _fm
from pathlib import Path as _Path

_FONT_DIR = _Path(__file__).resolve().parent.parent / "fonts" / "helvetica-255"
if _FONT_DIR.is_dir():
    for _ff in _FONT_DIR.glob("*.ttf"):
        _fm.fontManager.addfont(str(_ff))
    for _ff in _FONT_DIR.glob("*.otf"):
        _fm.fontManager.addfont(str(_ff))

# Verifica se Helvetica foi registrada; senão, usa DejaVu Sans como fallback
_helvetica_found = any(
    f.name.lower().startswith("helvetica")
    for f in _fm.fontManager.ttflist
)
rcParams['font.family'] = 'Helvetica' if _helvetica_found else 'DejaVu Sans'
...
QUIMICA_CORES = [AZUL, AMARELO_JOHN_DEERE, AMARELO_ALERTA, VERMELHO]
...
def salvar_pdf_a3_desempenho(nome_arquivo: str, secoes: list[dict], fmt_tabela: dict) -> None:
    ...
    if not any(func_consulta_possui_registros(secao.get("df_tabela")) for secao in secoes):
   ...
   # ------------------------------------------------------------------------
    ...
    if consulta_tem_registros["alertas"]:
        ...
        secoes_alertas = [
            ...
            {"df": alertas_criticos_moto, "bg": PATH_PAGINAS_ALERTAS["tabelas_criticas_carre"]},
            {"df": alertas_criticos_escav, "bg": PATH_PAGINAS_ALERTAS["tabelas_criticas_escav"]},
            {"df": alertas_criticos_carreg, "bg": PATH_PAGINAS_ALERTAS["tabelas_critica_moto"]},
        ]
    ...
    arquivo_principal = f"relatorio_{cliente_id}_{datas_relatorio['data_inicial_formatada']}_{datas_relatorio['data_final_formatada']}.pdf"
    ...
    paragrafo_periodo = (
        ...
        f"{datas_relatorio['data_inicial_texto']} ate {datas_relatorio['data_final_texto']}."
    )
    ...
    if consulta_tem_registros["garantia"]:
        ...
        func_criar_pagina_sem_dados(
            nome_arquivo=ARQUIVOS_SECOES_PDF["analise_garantia"],
            formato_pagina=A4,
            margins=(3 * cm, 2 * cm, 2 * cm, 2 * cm),
            background_image="labels/10_0_pag.png",
            spacer_cm=10,
            font_size=24,
        )
...
def rda_py_data_fixa(cliente_id: int):
    ...
    return rda_py_gerar(
        ...
        data_inicial=datas_relatorio["data_inicial"],
        data_final=datas_relatorio["data_final"],
        datas_relatorio=datas_relatorio,
    )
...
def rda_py_by_pin(chassi: str, data_inicial: str, data_final: str):
    ...
    bandeira_estado = r'icons\flags\PE.png'
...
if __name__ == "__main__":
    ...
    print(YELLOW + r"""
                        ...
                        ██╔══██╗██║  ██║██╔══██║
                        ██║  ██║██████╔╝██║  ██║
        |RDA - RELATÓRIOS DE DESEMPENHO AUTOMÁTICOS - Versão 0.3 - 26/05/2026 - Versão Estável.
